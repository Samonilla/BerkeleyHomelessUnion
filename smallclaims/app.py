...
def _build_ag_complaint_docx(data: dict) -> bytes:
...
def _render_ag_complaint_form(prefix: str, *, default_entity: str = "", default_location: str = "", default_subject: str = "") -> None:
    c1, c2 = st.columns(2)
    with c1:
    ...
    if st.button("Generate AG Complaint Draft", type="primary", use_container_width=True, key=f"{prefix}_gen_ag"):
        if not complainant_name.strip() or not subject.strip() or not details.strip() or not entity.strip():
            ...
            st.session_state[f"{prefix}_ag_docx"] = _build_ag_complaint_docx(payload)
            ...
            st.session_state[f"{prefix}_ag_filebase"] = _slug(payload["name"] or "ag_complaint")
    ...
    if st.session_state.get(f"{prefix}_ag_docx"):
        ...
        _filebase = st.session_state.get(f"{prefix}_ag_filebase", "ag_complaint")
        st.download_button(
            "⬇️ Download AG Complaint (Word)",
            data=st.session_state[f"{prefix}_ag_docx"],
            file_name=f"{_filebase}_ag_complaint.docx",
        ...
        st.download_button(
            ...
            file_name=f"{_filebase}_ag_complaint.txt",
            mime="text/plain",
            use_container_width=True,
